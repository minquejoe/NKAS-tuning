...
BUTTON_FILE = 'assets_game.py'  # 修改输出文件名为assets_game.py
IMPORT_EXP = """
...
class AssetExtractor:
    def __init__(self):
        logger.info('Minigame Assets extract')
        event = NikkeConfig.EVENTS[0].get('event_id')
        # 修改为minigame路径
        minigame_folder = os.path.join(NikkeConfig.ASSETS_FOLDER, 'minigame')
        modules = [m for m in os.listdir(minigame_folder) if
                   os.path.isdir(os.path.join(minigame_folder, m)) and m == event]
